...
def code_without_libraries(file):
    ...
    return ''.join([line for line in lines if not line.strip().startswith("import")])
...
def encryption_metode_1(code_to_encode):
    ...
    mesage_int = [ord(b) for b in code_to_encode]
    ct = [(prime_number * number + salt) % 256 for number in mesage_int]
